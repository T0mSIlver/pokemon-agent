"""Verifiable progress oracle for Pokemon Red.

Every milestone here resolves to a bit somewhere in RAM, so "how far along is
this run" is a memory read rather than a judgement call. The table itself comes
from the *pokered* decomp and is regenerated by ``scripts/gen_milestones.py``
into ``pokemon_agent/data/red_milestones.json``; nothing in this module reaches
the network.

Four kinds of milestone, distinguished by ``Milestone.kind``:

* ``event``   -- a bit in ``wEventFlags`` (0xD747). ``source`` is the pokered
  ``EVENT_`` name and ``ALL_EVENTS`` maps it to its bit index.
* ``badge``   -- a bit in ``wObtainedBadges`` (0xD356). ``source`` is ``badge_bit:N``.
* ``item``    -- a key item held in the bag *or* the item PC, ``item_id:N``.
* ``ram_bit`` -- one bit of one WRAM byte, ``ram_bit:0xADDR:N``. Two rungs, both
  at the end of the game, where every event flag pokered defines is scratch
  space the scripts clear again. See ``RESETTABLE_EVENTS`` below.

A rung has to be *monotone*: once banked it stays banked, or the score is not a
score. That is not a property of event flags in general -- pokered clears forty
of them -- so ``RESETTABLE_EVENTS`` is scanned out of the decomp by
``scripts/gen_milestones.py`` and no rung is allowed to use one. The list used to
be typed by hand and was eleven names short, and the eleven included the last
eight rungs: Route23.asm clears the Victory Road boulder switches on every map
load, IndigoPlateauLobby.asm clears the whole Elite Four block when a failed
challenge sends you back to the lobby, HallOfFame.asm clears it again on the way
in, and the Hall of Fame flag itself is consumed by the Pokedex rating in the
same cutscene that sets it. Beating the game moved the score *down* by eight and
the top of the ladder could not be reached at all.

Bit order inside ``wEventFlags`` is little-endian per byte, matching pokered's
``flag_array``: event *i* lives in byte ``i // 8`` at mask ``1 << (i % 8)``. That
is not an assumption. Two independent confirmations:

* ``pokemon_agent/memory/red.py`` already carried ``ADDR_POKEDEX_FLAG = 0xD74B``
  bit 5 and ``ADDR_OAK_PARCEL = 0xD74E`` bit 1, read off a running game years
  before this table existed. EVENT_GOT_POKEDEX parses as bit 37 (byte 4, bit 5)
  and EVENT_GOT_OAKS_PARCEL as bit 57 (byte 7, bit 1). Both land exactly there.
* Sweeping all 3277 save states in ``saves/``: ``wObtainedBadges`` bit 0 (Boulder
  Badge) and EVENT_BEAT_BROCK (bit 119 -> byte 14, mask 0x80) agree in every
  single one, 444 set and the rest clear, zero disagreements. The big-endian
  reading puts EVENT_BEAT_BROCK at byte 14 mask 0x01, which is clear in all 444
  badge-holding states -- so it is not merely unconfirmed, it is refuted.
"""

from __future__ import annotations

import json
from dataclasses import dataclass
from pathlib import Path
from types import MappingProxyType
from typing import Any, Collection, Dict, List, Mapping, Tuple

from pokemon_agent.memory.red import ADDR_EVENT_FLAGS, BADGE_NAMES

DATA_PATH = Path(__file__).resolve().parent / "data" / "red_milestones.json"

#: wEventFlags is NUM_EVENTS = 0xA00 bits wide.
EVENT_FLAG_BYTES = 0xA00 // 8

#: How many trailing ladder entries ``summary()["latest"]`` reports.
LATEST_LIMIT = 5


@dataclass(frozen=True)
class Milestone:
    id: str
    label: str
    kind: str  # "event" | "badge" | "item" | "ram_bit"
    source: str
    ladder_index: int  # -1 if not on the curated ladder


def _load() -> Tuple[Tuple[Milestone, ...], Mapping[str, int], frozenset[str]]:
    document = json.loads(DATA_PATH.read_text(encoding="utf-8"))
    events = {entry["id"]: entry["bit_index"] for entry in document["events"]}
    ladder = tuple(
        Milestone(
            id=entry["id"],
            label=entry["label"],
            kind=entry["kind"],
            source=entry["source"],
            ladder_index=index,
        )
        for index, entry in enumerate(document["ladder"])
    )
    return ladder, MappingProxyType(events), frozenset(document["resettable"])


MILESTONES: Tuple[Milestone, ...]
ALL_EVENTS: Mapping[str, int]
#: Every ``EVENT_`` name pokered can clear again, scanned out of the decomp. A
#: flag in here is not progress, it is state, and may not be a rung.
RESETTABLE_EVENTS: frozenset[str]
MILESTONES, ALL_EVENTS, RESETTABLE_EVENTS = _load()

MILESTONES_BY_ID: Mapping[str, Milestone] = MappingProxyType({m.id: m for m in MILESTONES})


def milestone_for_event(event_id: str) -> Milestone:
    """Milestone view of any ``EVENT_`` name, on the ladder or not.

    Off-ladder events get ``ladder_index == -1``: they are still verifiable, they
    just do not move the score.
    """
    known = MILESTONES_BY_ID.get(event_id)
    if known is not None and known.kind == "event":
        return known
    if event_id not in ALL_EVENTS:
        raise KeyError(event_id)
    return Milestone(id=event_id, label=event_id, kind="event", source=event_id, ladder_index=-1)


def _source_number(source: str) -> int:
    return int(source.split(":", 1)[1])


def _ram_bit(source: str) -> Tuple[int, int]:
    """``ram_bit:0xD70B:9`` -> ``(0xD70B, 9)``.

    The bit index runs past a byte boundary the way pokered's ``flag_array`` does:
    bit 9 is byte 1, bit 1, LSB first. Same order as :meth:`read_bits`.
    """
    _, address, bit = source.split(":")
    return int(address, 16), int(bit)


class MilestoneTracker:
    """Reads the curated ladder out of a live game.

    Parameters
    ----------
    reader : PokemonRedReader
        Any :class:`~pokemon_agent.memory.red.RedBlueMemoryReader`.
    """

    def __init__(self, reader: Any) -> None:
        self.reader = reader

    # -- reads --------------------------------------------------------------

    def _event_bits(self) -> List[bool]:
        # read_bits is already LSB-first per byte, which is pokered's flag_array order.
        return self.reader.read_bits(ADDR_EVENT_FLAGS, EVENT_FLAG_BYTES)

    def _badge_bits(self) -> set[int]:
        obtained = set(self.reader.read_flags()["badges"])
        return {bit for bit, name in enumerate(BADGE_NAMES) if name in obtained}

    def _held_item_ids(self) -> set[int]:
        """Bag and item PC together.

        Gen 1 has no key-item pocket: every one of the four item rungs can be
        deposited in the PC, and a rung that only looked in the bag would go out
        again when the player tidied up. Reading both is what makes an item
        milestone as monotone as an event one.
        """
        held = {entry["id"] for entry in self.reader.read_bag()}
        read_pc = getattr(self.reader, "read_pc_items", None)
        if read_pc is not None:
            held.update(entry["id"] for entry in read_pc())
        return held

    # -- public interface ---------------------------------------------------

    def snapshot(self) -> frozenset[str]:
        """Ids of every ladder milestone currently satisfied."""
        bits = self._event_bits()
        badges = self._badge_bits()
        items = self._held_item_ids()

        reached: List[str] = []
        for milestone in MILESTONES:
            if milestone.kind == "event":
                index = ALL_EVENTS[milestone.source]
                ok = index < len(bits) and bits[index]
            elif milestone.kind == "badge":
                ok = _source_number(milestone.source) in badges
            elif milestone.kind == "ram_bit":
                address, bit = _ram_bit(milestone.source)
                ok = self.reader.read_bits(address, bit // 8 + 1)[bit]
            else:
                ok = _source_number(milestone.source) in items
            if ok:
                reached.append(milestone.id)
        return frozenset(reached)

    def newly_set(self, previous: frozenset[str]) -> Tuple[str, ...]:
        """Milestones reached since *previous*, in ladder order.

        An empty *previous* therefore yields everything currently set, which is
        what a fresh tracker attached to a loaded save state should report.
        """
        current = self.snapshot()
        gained = current - previous
        return tuple(m.id for m in MILESTONES if m.id in gained)

    def summary(self) -> Dict[str, Any]:
        """Scoreboard for the current state.

        ``furthest`` is the highest ladder index reached, not the most recent
        thing to happen: the ladder is the scoring axis and ``max()`` over it is
        monotone even when a player picks up an early milestone late.
        """
        current = self.snapshot()
        reached = [m for m in MILESTONES if m.id in current]
        furthest = reached[-1] if reached else None
        return {
            "count": len(reached),
            "total": len(MILESTONES),
            "furthest": furthest.id if furthest else None,
            "furthest_index": furthest.ladder_index if furthest else -1,
            "latest": [m.id for m in reached[-LATEST_LIMIT:]],
            # Computed off the same snapshot rather than a second read, so the
            # score and what is open next can never describe different moments.
            "frontier": [m.id for m in frontier(current)],
        }

    def frontier(self) -> Tuple[Milestone, ...]:
        """Milestones the game will let the player attempt from where it is now."""
        return frontier(self.snapshot())


# ---------------------------------------------------------------------------
# The plan the ladder always was
#
# The 58 rungs above were only ever a scoreboard: a flat list, read after the
# fact. Read the other way round they are a plan nobody wrote down. One run
# spent 457 tool calls and fourteen hours banking zero of them -- 361 `act`, 64
# `state`, and not a single `route`, `frontier`, `calc` or `progress` -- because
# the model was handed twenty verbs and an open map and collapsed to the two
# cheapest. Writing the orderings down turns the list into a graph, and a graph
# has a frontier: the few milestones whose preconditions already hold.
#
# What is deliberately *not* here is any notion of which frontier node to take.
# The harness narrows the menu; the model still orders off it.
#
# Two rules govern the table.
#
# 1. A postcondition is never asserted, only read. Each node's postcondition is
#    the milestone's own ``kind``/``source`` -- an event bit, a badge bit, a bag
#    slot -- so the frontier is a memory read like every other answer in this
#    module. Nothing here marks itself done.
# 2. An uncertain edge is left out. A precondition that is wrong in the strict
#    direction hides a legal goal, which is the failure this structure exists to
#    prevent; a missing one only offers something not yet reachable, which costs
#    one wasted look. So these are edges the game enforces -- a blocked road, a
#    locked door, a badge check the field move will fail -- and soft "you would
#    normally do this first" orderings are absent on purpose.
# ---------------------------------------------------------------------------

#: Gen 1 gates field moves on badges rather than on knowing the move, so the
#: ground behind a tree needs HM01 *and* the Cascade Badge. Surf is the same
#: shape. These pairs are the joins that make the chain a graph.
CUT = ("EVENT_GOT_HM01", "BADGE_CASCADE")
SURF = ("EVENT_GOT_HM03", "BADGE_SOUL")

#: Route 23's guards check one badge each on the climb to Victory Road.
ALL_BADGES = (
    "BADGE_BOULDER",
    "BADGE_CASCADE",
    "BADGE_THUNDER",
    "BADGE_RAINBOW",
    "BADGE_SOUL",
    "BADGE_MARSH",
    "BADGE_VOLCANO",
    "BADGE_EARTH",
)


@dataclass(frozen=True)
class MilestoneNode:
    """One rung as a planning step rather than a scoreboard row.

    ``requires`` is a conjunction: every id in it must be satisfied before this
    milestone is on the frontier. ``effects`` say what the world gains, in the
    terms a player would use -- a road that opens, a move that starts working
    outside battle -- because "what does this get me" is what picks between two
    open goals.

    ``excludes`` covers the one place Red forks irreversibly: take the Dome
    Fossil and the Helix is gone for the run. Without it the road not taken
    would sit on the frontier forever, unreachable and still advertised.

    There is no postcondition field. The postcondition is the milestone itself,
    read out of RAM by :class:`MilestoneTracker`.
    """

    id: str
    requires: Tuple[str, ...]
    effects: Tuple[str, ...]
    excludes: Tuple[str, ...] = ()


#: ``id -> (requires, effects, excludes)``, in ladder order.
_PLAN: Dict[str, Tuple[Tuple[str, ...], Tuple[str, ...], Tuple[str, ...]]] = {
    "EVENT_GOT_STARTER": ((), ("a Pokemon of your own",), ()),
    # The rival challenges you on the way out of the lab; it is not optional.
    "EVENT_BATTLED_RIVAL_IN_OAKS_LAB": (("EVENT_GOT_STARTER",), (), ()),
    "EVENT_GOT_OAKS_PARCEL": (("EVENT_BATTLED_RIVAL_IN_OAKS_LAB",), (), ()),
    "EVENT_OAK_GOT_PARCEL": (("EVENT_GOT_OAKS_PARCEL",), (), ()),
    # The old man asleep in the road north out of Viridian wants his coffee, and
    # ViridianCity.asm checks EVENT_GOT_POKEDEX to decide whether he is still
    # lying there -- not the parcel delivery, which happens a beat earlier. The
    # two are one conversation apart, which is exactly why the effect sat on the
    # wrong rung until someone read the script.
    "EVENT_GOT_POKEDEX": (
        ("EVENT_OAK_GOT_PARCEL",),
        ("the Pokedex", "the road north out of Viridian"),
        (),
    ),
    "EVENT_GOT_TOWN_MAP": (("EVENT_GOT_POKEDEX",), ("where every town is",), ()),
    "EVENT_BEAT_ROUTE22_RIVAL_1ST_BATTLE": (("EVENT_GOT_POKEDEX",), (), ()),
    "EVENT_BEAT_BROCK": (("EVENT_GOT_POKEDEX",), ("the road east out of Pewter",), ()),
    "BADGE_BOULDER": (("EVENT_BEAT_BROCK",), ("Flash outside battle",), ()),
    # Optional: the fossil room is on Mt. Moon B2F, which warps only back to
    # B1F, while the way out to Route 4 sits on B1F. So nothing east of the
    # mountain waits on this fight.
    "EVENT_BEAT_MT_MOON_EXIT_SUPER_NERD": (("EVENT_BEAT_BROCK",), ("the two fossils",), ()),
    "EVENT_GOT_DOME_FOSSIL": (
        ("EVENT_BEAT_MT_MOON_EXIT_SUPER_NERD",),
        (),
        ("EVENT_GOT_HELIX_FOSSIL",),
    ),
    "EVENT_GOT_HELIX_FOSSIL": (
        ("EVENT_BEAT_MT_MOON_EXIT_SUPER_NERD",),
        (),
        ("EVENT_GOT_DOME_FOSSIL",),
    ),
    # He waits at the north end of Cerulean in front of Nugget Bridge, not
    # between the player and the gym, so Misty does not wait on him.
    "EVENT_BEAT_CERULEAN_RIVAL": (("EVENT_BEAT_BROCK",), ("Nugget Bridge and Route 25",), ()),
    "EVENT_BEAT_MISTY": (("EVENT_BEAT_BROCK",), (), ()),
    "BADGE_CASCADE": (("EVENT_BEAT_MISTY",), ("Cut outside battle",), ()),
    "EVENT_MET_BILL": (("EVENT_BEAT_CERULEAN_RIVAL",), (), ()),
    "EVENT_GOT_SS_TICKET": (("EVENT_MET_BILL",), ("passage aboard the S.S. Anne",), ()),
    "EVENT_RUBBED_CAPTAINS_BACK": (("EVENT_GOT_SS_TICKET",), (), ()),
    "EVENT_GOT_HM01": (
        ("EVENT_RUBBED_CAPTAINS_BACK",),
        ("trees, once the Cascade Badge allows it",),
        (),
    ),
    "EVENT_SS_ANNE_LEFT": (("EVENT_GOT_HM01",), (), ()),
    # A small tree stands in front of the gym door, so Surge needs Cut working.
    "EVENT_BEAT_LT_SURGE": (CUT, (), ()),
    "BADGE_THUNDER": (("EVENT_BEAT_LT_SURGE",), ("Fly outside battle",), ()),
    # The museum is in Pewter, which is behind the same sleepy old man.
    "EVENT_GOT_OLD_AMBER": (("EVENT_GOT_POKEDEX",), (), ()),
    "EVENT_GOT_BIKE_VOUCHER": (("EVENT_BEAT_BROCK",), (), ()),
    "EVENT_GOT_BICYCLE": (("EVENT_GOT_BIKE_VOUCHER",), ("the bicycle, and Cycling Road",), ()),
    # Oak's aide also wants ten species in the Pokedex, which is a count rather
    # than a milestone and so cannot be an edge here.
    "EVENT_GOT_HM05": (
        ("EVENT_GOT_POKEDEX",),
        ("dark caves, once the Boulder Badge allows it",),
        (),
    ),
    # Celadon lies behind the Cut trees east of Cerulean: Route 9, Rock Tunnel,
    # Lavender, then the Route 7-8 Underground Path around sealed Saffron.
    "EVENT_FOUND_ROCKET_HIDEOUT": (CUT, (), ()),
    "ITEM_LIFT_KEY": (("EVENT_FOUND_ROCKET_HIDEOUT",), ("the hideout lift",), ()),
    "EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI": (("EVENT_FOUND_ROCKET_HIDEOUT",), (), ()),
    "ITEM_SILPH_SCOPE": (
        ("EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI",),
        ("sight of the Marowak ghost",),
        (),
    ),
    "EVENT_BEAT_ERIKA": (CUT, (), ()),
    "BADGE_RAINBOW": (("EVENT_BEAT_ERIKA",), ("Strength outside battle",), ()),
    "EVENT_BEAT_POKEMON_TOWER_RIVAL": (CUT, (), ()),
    "EVENT_BEAT_GHOST_MAROWAK": (
        ("ITEM_SILPH_SCOPE", "EVENT_BEAT_POKEMON_TOWER_RIVAL"),
        (),
        (),
    ),
    "EVENT_RESCUED_MR_FUJI": (("EVENT_BEAT_GHOST_MAROWAK",), (), ()),
    "EVENT_GOT_POKE_FLUTE": (("EVENT_RESCUED_MR_FUJI",), ("both Snorlax roads south",), ()),
    "EVENT_BEAT_ROUTE12_SNORLAX": (("EVENT_GOT_POKE_FLUTE",), (), ()),
    # The house holding HM02 sits behind a tree on Route 16, west of Celadon.
    "EVENT_GOT_HM02": (CUT, ("Fly, once the Thunder Badge allows it",), ()),
    # Fuchsia is past one Snorlax or the other; either way that is the flute.
    "EVENT_GOT_HM03": (("EVENT_GOT_POKE_FLUTE",), ("water, once the Soul Badge allows it",), ()),
    "EVENT_GAVE_GOLD_TEETH": (("EVENT_GOT_POKE_FLUTE",), (), ()),
    "EVENT_GOT_HM04": (
        ("EVENT_GAVE_GOLD_TEETH",),
        ("boulders, once the Rainbow Badge allows it",),
        (),
    ),
    "EVENT_BEAT_KOGA": (("EVENT_GOT_POKE_FLUTE",), (), ()),
    "BADGE_SOUL": (("EVENT_BEAT_KOGA",), ("Surf outside battle",), ()),
    # Saffron opens on a drink bought from the Celadon vending machines, so
    # everything inside it stands behind reaching Celadon at all.
    "ITEM_CARD_KEY": (CUT, ("the locked doors in Silph Co.",), ()),
    "EVENT_BEAT_SILPH_CO_RIVAL": (CUT, (), ()),
    "EVENT_BEAT_SILPH_CO_GIOVANNI": (("ITEM_CARD_KEY",), ("the Viridian Gym",), ()),
    "EVENT_GOT_MASTER_BALL": (("EVENT_BEAT_SILPH_CO_GIOVANNI",), (), ()),
    "EVENT_BEAT_SABRINA": (CUT, (), ()),
    "BADGE_MARSH": (("EVENT_BEAT_SABRINA",), (), ()),
    # Cinnabar is an island. Surf is the only way onto it.
    "ITEM_SECRET_KEY": (SURF, ("the Cinnabar Gym door",), ()),
    "EVENT_BEAT_BLAINE": (("ITEM_SECRET_KEY",), (), ()),
    "BADGE_VOLCANO": (("EVENT_BEAT_BLAINE",), (), ()),
    # Giovanni is only in his own gym once he has been run out of Silph Co.
    "EVENT_BEAT_VIRIDIAN_GYM_GIOVANNI": (("EVENT_BEAT_SILPH_CO_GIOVANNI",), (), ()),
    "BADGE_EARTH": (("EVENT_BEAT_VIRIDIAN_GYM_GIOVANNI",), ("every traded Pokemon obeys",), ()),
    "EVENT_BEAT_ROUTE22_RIVAL_2ND_BATTLE": (("BADGE_EARTH",), (), ()),
    # Five guards up Route 23, one badge checked each. The last of them is the
    # Earth Badge check, so this flag is the whole road in one read. The rival
    # waiting on Route 22 is on the way but is a coordinate trigger rather than a
    # wall, so it is not an edge.
    "EVENT_PASSED_EARTHBADGE_CHECK": (ALL_BADGES, ("the Victory Road gate",), ()),
    # Strength shifts the boulders onto the floor switches inside; there is no
    # way through without it. Indigo Plateau has no other land route, so arriving
    # there at all is the read that says Victory Road is behind you.
    "TOWN_INDIGO_PLATEAU": (
        ("EVENT_PASSED_EARTHBADGE_CHECK", "EVENT_GOT_HM04"),
        ("the Elite Four",),
        (),
    ),
    "ELITE_FOUR_CHAMPION": (("TOWN_INDIGO_PLATEAU",), ("Cerulean Cave",), ()),
}

MILESTONE_DAG: Mapping[str, MilestoneNode] = MappingProxyType(
    {
        milestone.id: MilestoneNode(
            id=milestone.id,
            requires=_PLAN[milestone.id][0],
            effects=_PLAN[milestone.id][1],
            excludes=_PLAN[milestone.id][2],
        )
        for milestone in MILESTONES
    }
)


def frontier(reached: Collection[str]) -> Tuple[Milestone, ...]:
    """Ladder milestones that are open right now, in ladder order.

    Open is three things at once: not already reached, every ``requires``
    reached, and no rival of an ``excludes`` pair taken. *reached* is whatever
    :meth:`MilestoneTracker.snapshot` read out of RAM -- this function judges
    nothing of its own, it intersects a set with a graph.

    Ids in *reached* that are not on the ladder are ignored rather than
    rejected, so an off-ladder event snapshot is safe to pass in.
    """
    have = frozenset(reached)
    open_now: List[Milestone] = []
    for milestone in MILESTONES:
        if milestone.id in have:
            continue
        node = MILESTONE_DAG[milestone.id]
        if not have.issuperset(node.requires):
            continue
        if have.intersection(node.excludes):
            continue
        open_now.append(milestone)
    return tuple(open_now)


def blocking(milestone_id: str, reached: Collection[str]) -> Tuple[str, ...]:
    """Which of *milestone_id*'s preconditions are still outstanding.

    The frontier says what is open; this says why something is not, which is
    the question that gets asked next.
    """
    have = frozenset(reached)
    return tuple(need for need in MILESTONE_DAG[milestone_id].requires if need not in have)
