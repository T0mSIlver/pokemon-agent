...
| `autopilot.py` + Hermes brain | **Reject** | Competes directly with `pi_supervisor.py`. Upstream drives a `hermes chat` subprocess with free-form prose and expects the model to curl back into the server; session ID is recovered by regex-scraping stdout. Adopting it would mean two incompatible brains. |
